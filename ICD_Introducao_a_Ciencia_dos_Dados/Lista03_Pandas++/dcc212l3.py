...
# %% [markdown]
# A célula abaixo apenas configura nosso notebook para ficar mais parecido com os das aulas
...
# %% [markdown]
# O método `read_csv` do `DataFrame` é usado para carregar dados de arquivos de texto. Como você pode ver no código de exemplo, é possível especificar opções como o delimitador de coluna e qual linha (se houver) contém cabeçalhos de coluna. Neste caso, o delimitador é uma vírgula e a primeira linha contém os nomes das colunas.
# 
...
# %% [markdown]
# Obseve como a última linha não possui o número de horas estudadas. Nas duas últimas, faltam as notas. Caso você queira saber a quantidade de dados faltantes, basta somar os `True`s da tabela acima. Aqui é importante saber que Python trata `True` de forma similar ao número 1. Portanto, basta somar a tabela inteira para pegar tal quantidade de dados faltantes.
...
# #### fillna
# 
# Uma abordagem comum é imputar valores de substituição. Por exemplo, se o número de horas de estudo está faltando, podemos simplesmente supor que o aluno estudou por um período médio de tempo e substituir o valor faltante com as horas de estudo médias. Para fazer isso, podemos usar o método fillna:
...
# %% [markdown]
# Para alterar, podemos trocar a coluna. Abaixo faço tal operação em uma cópia dos dados, para não mudar a tabela original.
...
# %% [markdown]
# Obserbe como acima temos uma entrada verdadeira. Esta, é justamente a linha onde temos o nome `Skye`. Ao indexar o DataFrame com tal linha, pegamos a nota da discente.
...
# %% [markdown]
# Como também os alunos que passaram estudando relativamente pouco.  No nosso caso, vamos focar em alunos que estudaram menos do que 14 horas.

# %%
df.query('Grade >= 60 and StudyHours <= 14')

# %% [markdown]
# Todos retornos, ou do índice booleano ou da query, são outros DataFrames. Então, podemos chamar métodos como tirar a média dos alunos.
...
#   * A mediana: o valor no meio do intervalo (ordenado) de todos os valores de amostra.
...
# 
# Agora temos uma boa ideia de onde estão as estatísticas centrais dos dados. No entanto, há outro aspecto das distribuições que devemos examinar: quanta variabilidade existe nos dados?
# 
...
# 
# Abaixo eu agrupo os dados por passou ou não. Depois, pego as horas de estudo. Por fim, observo as estatísticas.
...
# 
# O método `.select_dtypes()` pode ser útil.

# %%
def all_median(df):
    return df.median().astype(int)

# %%
assert_equal(2013, all_median(df.dropna())['Year'])
assert_equal(7, all_median(df.dropna())['Month'])
...
# %%
assert_almost_equal(10.456614371849216, delay(df.dropna())[0])
assert_almost_equal(6.563286883449304, delay(df.dropna())[1])
...
# %%
def high_delay(df):
    #1° filtramos os voos cancelados
    NCancelados = df.dropna().query('Cancelled == 0')[["DestAirportName","OriginAirportName","CRSDepTime","CRSArrTime"]].copy()
    #2° criamos uma coluna pro tempo de voo
    NCancelados['TempoDeVoo'] = np.zeros(NCancelados.shape[0])
    """
    3° calculamos o tempo de voo
    Quando a hora de chegada programada for maior do que a de partida temos voos no msm dia
    Nesse caso basta subtrair o horario de saida do de chegada
    Caso contrário, subtraimos o horario de partida de 24h (2400) e somamos o horario de chegada
    """
    viajemNoMesmoDia = NCancelados["CRSArrTime"] >= NCancelados["CRSDepTime"]
    NCancelados.loc[viajemNoMesmoDia, 'TempoDeVoo'] = (NCancelados["CRSArrTime"]) - (NCancelados["CRSDepTime"])
    NCancelados.loc[~viajemNoMesmoDia, 'TempoDeVoo'] = 2400 - NCancelados["CRSDepTime"] + NCancelados["CRSArrTime"]

    #4° altera o df para mostrar apenas o nome do aeroporto de origem e destino
    NCancelados = NCancelados[['OriginAirportName','DestAirportName','TempoDeVoo']]

    #5° agrupa os voos de msm rota e calcula a media
    rotaDeMaiorTempo = NCancelados.groupby(['OriginAirportName','DestAirportName']).mean()

    #6° ordena os valores pelo tempo, e retorna o resultado
    rotaDeMaiorTempo = rotaDeMaiorTempo.sort_values(by='TempoDeVoo').iloc[-1].name
    
    return rotaDeMaiorTempo
...
# %%
atrasados = df.query('DepDelay != 0')
atrasados.boxplot(column='DepDelay', by='DayOfWeek', grid=False, showfliers=False)
...
# %%
NCancelados = df.dropna().query('Cancelled == 0').copy()
plt.scatter(NCancelados.DepDelay, NCancelados.ArrDelay, edgecolor='k', alpha=0.75)
plt.xlabel('DepDelay')
plt.ylabel('ArrDelay')
