...
# 
# Vamos focar numa base de 3 pontos. Além do mais, vamos assumir uma reta qualquer, dessa forma, o desvio padrão dos erros podem ser estimados através dos dados após a regressão. Se os erros vêm de uma distribuição normal com função de densidade já conhecida, qual dos três erros representados na figura abaixo é o mais verossímil?
# 
# Observe:
# $$p(\epsilon_i|\sigma^2, \alpha, \beta) = \frac{1}{\sqrt{\sigma^2 2 \pi}} e^{-\frac{(y_i - \beta x_i - \alpha)^2}{2 \sigma^2}}$$
# 
...
def error(alpha, beta, x, y):
    predicted = (x * beta) + alpha
    return y - predicted

def sum_of_squared_errors(alpha, beta, x, y):
    sum_of_errors = error(alpha, beta, x, y)
    squared_values = pow(sum_of_errors,2)
    return squared_values.sum()

def total_sum_of_squares(y):
    mean = y.mean()
    mean_centered = y - mean
    squared_values = pow(mean_centered,2)
    return squared_values.sum()
...
def linear_regression(x, y):
  ...
  # (alpha, beta)
  regression = ss.linregress(x,y)
  tupla = regression[1], regression[0]
  return tupla
...
def error_verification(alpha, beta, x, y):
  ...
  below_zero = errors < 0
  above_zero = errors > 0
  maiores = above_zero.sum()
  menores = below_zero.sum()
...
# avaliacao via grafico dos residuos (sem correcao automatica)
# seu codigo aqui
ss.probplot(y - y.mean(), plot=plt.gca())
# gabarito a seguir
...
# %% [markdown]
# Imagine que observamos variáveis categóricas $X$. $X$ corresponde ao efeito do remédio em um grupo de pessoas, onde cada $x_i$ assume valo igual a 1, caso o remédio tenha funcionado e 0 caso contrário. Por exemplo: $X = \{1, 0, 1, 1, 0, 1, 1, 1, 0, 0, 1, 0\}$
# 
...
# 
# $$l(X|\theta) = \log\left(\prod_i p(x_i|\theta)\right) = \sum_i \log\left(\theta^{x_i} (1-\theta)^{(1 - x_i)}\right) = \sum_i x_i \log(\theta) + \sum_i (1-x_i) \log(1-\theta)$$
# lembrando que $\log(ab) = \log(a) + \log(b)$ e $\log(a^x) = x\log(a)$
...
# %% [markdown]
# Assumimos que as observações são independentes, maximizar a log da verossimilhança corresponde a derivar essa função e igualar a zero, encontrando o seu ponto de máximo.
# 
